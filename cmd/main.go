...
func main() {
	...
	serviceWatcher := watcher.NewServiceWatcher(mgr.GetCache())
	setupLog.Info("adding service watcher to manager")
	if err := mgr.Add(serviceWatcher); err != nil {
		setupLog.Error(err, "unable to add service watcher to manager")
		os.Exit(1)
	}
